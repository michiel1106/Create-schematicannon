package com.bikerboys.schematicannon.impl.registrate;
...
public class CreateRegistrateRegistrationCallbackImpl {
	...
	private static final Map<String, Either<List<CallbackImpl<?, ?>>, SchematicannonRegistrate>> CALLBACKS = new HashMap<>();

	public static void provideRegistrate(SchematicannonRegistrate registrate) {
	...
	public static <R, T extends R> void register(ResourceKey<? extends Registry<R>> registry, ResourceLocation id, NonNullConsumer<? super T> callback) {
		...
		Either<List<CallbackImpl<?, ?>>, SchematicannonRegistrate> either;
		synchronized (CALLBACKS) {
	...
	private record CallbackImpl<R, T extends R>(ResourceKey<? extends Registry<R>> registry, ResourceLocation id, NonNullConsumer<? super T> callback) {
		...
		public void addToRegistrate(SchematicannonRegistrate registrate) {
