		...
		context.enqueueWork(() -> {
			...
			if (code == BEGIN) {
				BlockPos pos = ((SchematicTableMenu) player.containerMenu).contentHolder
						.getBlockPos();
				Schematicannon.SCHEMATIC_RECEIVER.handleNewUpload(player, schematic, size, pos);
			}
			if (code == WRITE)
				Schematicannon.SCHEMATIC_RECEIVER.handleWriteRequest(player, schematic, data);
			if (code == FINISH)
				Schematicannon.SCHEMATIC_RECEIVER.handleFinishedUpload(player, schematic);
		});
		return true;
	}
