...
public class SchematicPromptScreen extends AbstractSimiScreen {
	...
	public void init() {
		...
		abort.withCallback(() -> {
			SchematicannonClient.SCHEMATIC_AND_QUILL_HANDLER.discard();
			onClose();
		});
	...
	private void confirm(boolean convertImmediately) {
		SchematicannonClient.SCHEMATIC_AND_QUILL_HANDLER.saveSchematic(nameField.getValue(), convertImmediately);
		onClose();
	}
}
